...
router.post('/newProducto', (req, res, next) => {
	if(req.body.data.emp_id == ""){
		req.body.data.emp_id = req.session.usuario.emp_id
	}
	_SQL.insertProducto(req.body.data,req.body.complementos).then((resp)=>{
		res.jsonp(resp)
	})
});
router.get('/listaProductos', (req, res, next) => {
	_SQL.listaProductos(req.session.usuario.emp_id).then((resp)=>{
		res.jsonp(resp)
	})
});
